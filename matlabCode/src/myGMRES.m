...
%   $Revision: 1.21.4.14 $ $Date: 2010/11/17 11:29:46 $
...
if strcmp(atype,'matrix')
    ...
    if ~isequal(size(b),[m,1])
        error('MATLAB:gmres:VectorSize',  ...
            'Right hand side must be a column vector of length %d to match the coefficient matrix.', m);
    end
else
    m = size(b,1);
    n = m;
...
if tol < eps
    warning('MATLAB:gmres:tooSmallTolerance', ...
        'Input tol is smaller than eps and may not be achieved by GMRES\n         Try to use a bigger tolerance');
    warned = 1;
    tol = eps;
elseif tol >= 1
    warning('MATLAB:gmres:tooBigTolerance', ...
        'Input tol is bigger than 1 \n         Try to use a smaller tolerance');
    warned = 1;
    tol = 1-eps;
end
...
if restarted
    outer = maxit;
    if restart > n
        warning('MATLAB:gmres:tooManyInnerIts', 'RESTART is %d but it should be bounded by SIZE(A,1).\n         Setting RESTART to %d.',restart, n);
        restart = n;
    end
    inner = restart;
else
    outer = 1;
    if maxit > n
        warning('MATLAB:gmres:tooManyInnerIts', 'MAXIT is %d but it should be bounded by SIZE(A,1).\n         Setting MAXIT to %d.',maxit, n);
        maxit = n;
    end
    inner = maxit;
end
...
if ((nargin >= 6) && ~isempty(M1))
    ...
    if strcmp(m1type,'matrix')
        if ~isequal(size(M1),[m,m])
            error('MATLAB:gmres:PreConditioner1Size', ...
                'Preconditioner must be a square matrix of size %d to match the problem size.', m);
        end
    end
else
    existM1 = 0;
...
if ((nargin >= 7) && ~isempty(M2))
    ...
    if strcmp(m2type,'matrix')
        if ~isequal(size(M2),[m,m])
            error('MATLAB:gmres:PreConditioner2Size', ...
                'Preconditioner must be a square matrix of size %d to match the problem size.', m);
        end
    end
else
...
if ((nargin >= 8) && ~isempty(x))
    if ~isequal(size(x),[n,1])
        error('MATLAB:gmres:XoSize', '%s%s', ...
            'Initial guess must be a column vector of length %d to match the problem size.', n);
    end
else
    x = zeros(n,1);
end
...
%            pause
            ...
            if normr_act <= tolb
                ...
                if moresteps >= maxmsteps
                    if ~warned
                        warning('MATLAB:gmres:tooSmallTolerance', ...
                            'Input tol might be obviously smaller than eps*cond(A) and may not be achieved by GMRES\n         Try to use a bigger tolerance');
                    end
                    flag = 3;
                    iter = [outiter, initer];
                    break;
